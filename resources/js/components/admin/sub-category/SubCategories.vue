...
export default {
  ...
  methods: {
    createCategory () {
      this.$router.push({path: 'sub-categories/create'});
    },
    editCategoryLink (id) {
      return `sub-categories/${id.toString()}/edit`
    },
    editCategory ( id ) {
    ...
    deleteCategory ( id ) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.post(process.env.MIX_APP_URL+'/api/sub-categories/'+id, { _method: 'DELETE' }, {
            headers: {
    ...
    getCategories (page){
      ...
      axios.get(process.env.MIX_APP_URL+'/api/sub-categories?page='+page, {
				headers: {
					'Authorization': 'Bearer '+localStorage.getItem("api_token") 
				}
			})
      .then(function (response) {
        self.items = response.data.all_subCategories;
      }).catch(function (error) {
        self.$router.push({ path: '/admin/login' });
      });
    },
  ...
  metaInfo: {
    ...
    meta: [
      {name: 'description', content: 'Manage All Sub Category.'}
    ]
  }
}
</script>
